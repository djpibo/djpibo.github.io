...
title: "Java에서 실제로 Bulk Insert 처리를 하는지 Oracle DB에서 직접 트레이스하기"
...
# Java에서 실제로 Bulk Insert 처리를 하는지 Oracle DB에서 직접 트레이스하기

매장에서는 하루 종일 발생한 입고/판매/재고이동/환불 등의 재고 관련 거래가 발생한다. 1,400개 매장에서 약 3만 건 상품 SKU 작업이 누적되면 하루 2천만 건에 달하는 장부를 기록하는 새벽 00시 배치가 실행된다.

이러한 대량 데이터를 INSERT하는 Step이 존재하는데, 애플리케이션 코드에서는 보통 리스트나 컬렉션 단위로 데이터를 묶어서 전달한다.  
Spring Data JPA의 `saveAll()`, MyBatis의 `<foreach>` 또는 `ExecutorType.BATCH`, JDBC의 `addBatch()`를 사용하면서 "DB 엔진에 Bulk로 전달되어 한 번에 Commit될 것"이라고 기대하는 편이다.

하지만 실제 Oracle 데이터베이스의 세션 통계나 SQL Trace를 확인해보면 기대와 전혀 다른 현상이 빈번하게 확인된다.  

10만 건의 데이터를 리스트로 넘겼음에도 Oracle 엔진은 10만 번의 개별 INSERT Call을 처리하느라 네트워크 패킷을 10만 번 주고받거나, 심지어 행마다 개별 Commit이 발생해 `log file sync` 대기 이벤트로 시스템 전체 I/O를 점유하기도 한다.

애플리케이션 계층의 배치 코드와 Oracle DB 엔진 내부의 물리적 실행 사이에 왜 이러한 차이점이 발생하는지, 그리고 이를 10046 Extended Trace와 딕셔너리 뷰를 통해 테이블 단위로 검증하는 방법을 정리한다.

---

## 1. Spring의 Oracle 데이터 액세스 기술 스택 조망

Spring 생태계에서 Oracle 데이터베이스에 접근하는 방식은 추상화 수준과 패러다임에 따라 크게 다음과 같이 구분된다. 실무에서는 단일 기술만 사용하기보다는, 일반 트랜잭션(OLTP)과 대량 배치(Batch)의 목적에 따라 기술을 조합하여 운용하는 편이다.
...
| | **jOOQ** | Java 코드 기반 Type-Safe SQL 빌더, Oracle 특화 구문 및 금융/정산 쿼리 |
| **Low Level / Core JDBC** | **Spring JdbcTemplate** | Spring 표준 JDBC 래퍼, **대용량 배치 적재(Spring Batch)의 표준** |
...
애플리케이션 개발자가 호출하는 메서드와 Oracle JDBC Thin Driver가 Net8 프로토콜을 통해 전송하는 물리적 패킷 사이에는 상당한 변환 계층이 존재한다.  
각 기술이 대량 INSERT를 내부적으로 어떻게 처리하며, 어떤 지점에서 Batching이 무력화되는지 살펴본다.

```mermaid
flowchart TB
    subgraph BAD["⚠️ 비효율: 단건 반복 전송 (Single Execution Loop)"]
        direction TB
    ...
    subgraph GOOD["✅ 권장: Native Array Processing (일괄 Batching)"]
        direction TB
        ...
        G_DB["Oracle 19c Database Engine<br/>• EXEC Call: 단 10회 (r=1000)<br/>• SQL*Net Roundtrip: 단 10회로 급감<br/>• 동일 블록 Pin 유지 연속 삽입 (cu 95% 절감)"]
...
JPA의 배치 처리는 영속성 컨텍스트의 **Write-Behind** 저장소에 INSERT 쿼리를 모아두었다가, 트랜잭션 커밋 직전 `flush()` 시점에 `hibernate.jdbc.batch_size` 설정에 맞춰 JDBC `executeBatch()`로 전달하는 구조다.

PK 채번 및 시퀀스가 포함될 경우 아래와 같은 함정이 발생한다:
- **`GenerationType.IDENTITY`**: Oracle 12c부터 지원하는 `IDENTITY` 컬럼을 매핑한 경우, 엔티티가 영속 상태가 되려면 1차 캐시의 식별자(PK)가 즉시 필요하다. 하지만 IDENTITY 전략은 DB에 실제 INSERT를 실행하기 전까지 ID 값을 알 수 없다. 따라서 Hibernate는 **Write-Behind를 강제로 비활성화**하고, `persist()` 호출 시점에 즉시 단건 INSERT 쿼리를 DB로 날린다. 결과적으로 `hibernate.jdbc.batch_size` 설정을 아무리 크게 주어도 완전히 무시되고 1건씩 단건 전송된다.
- **`GenerationType.SEQUENCE`**: Oracle 시퀀스를 사용하면 INSERT 실행 전에 시퀀스 값을 먼저 조회해 엔티티에 할당할 수 있으므로 쓰기 지연 Batching이 정상 동작한다. 단, 시퀀스의 `allocationSize`와 Hibernate 설정(`order_inserts=true`, `batch_size=1000`)을 일치시키지 않으면 시퀀스 조회를 위해 수만 번의 `SELECT ... NEXTVAL` Roundtrip이 발생한다.
...
- **Oracle 연동**: Oracle JDBC Thin Driver의 SDU(Session Data Unit) 버퍼에 2차원 바인드 배열이 패키징되어 한 번에 Oracle 서버로 전송된다. 엔티티 상태 관리나 쓰기 지연과 같은 부작용이 전혀 발생하지 않는다.

---

### 2.3. MyBatis: XML `<foreach>`의 구조적 한계 vs `ExecutorType.BATCH`
MyBatis 환경에서 대량 INSERT를 구현할 때 가장 흔히 시도하는 방식이 XML 동적 태그인 `<foreach>`다.  
Oracle은 `INSERT INTO table VALUES (...), (...)` 멀티플 로우 구문을 지원하지 않으므로, 흔히 `INSERT ALL` 과 같은 우회 구문을 사용한다.
...
이 방식의 구조적 한계:
1. **바인드 변수 한도 초과**: Oracle의 최대 바인드 파라미터 개수는 65,535개다. 컬럼이 10개인 테이블이라면 6,500건을 넘기는 순간 `ORA-01745: invalid host/bind variable name` 에러가 발생한다.
2. **Hard Parse 및 라이브러리 캐시 래치 경합 발생**: 리스트 크기에 따라 매번 SQL 텍스트의 길이가 달라지고 바인드 변수 개수가 달라져 Hard Parse가 발생하며 Shared Pool 래치(`library cache: mutex X`)가 경합한다.

MyBatis에서 Oracle의 Native Batching을 활용하는 적합한 방법은 단건 INSERT XML 구문을 따로 작성해두고, `SqlSessionTemplate`을 `ExecutorType.BATCH` 모드로 실행하는 것이다.
...
jOOQ는 Java 코드로 SQL을 작성하므로 컴파일 타임 검증이 가능하며, 대량 적재 시 `batchInsert()` 메서드를 통해 Native JDBC Batching을 깔끔하게 지원한다.
...
- **내부 메커니즘**: 전달된 `Record` 목록을 기반으로 단일 INSERT 템플릿 Statement를 생성하고, `executeBatch()`를 호출하여 Oracle Thin Driver의 Array Processing을 활성화한다.
- **Oracle 특화 최적화**: 대량 적재 시 Direct-Path Insert 힌트인 `/*+ APPEND */`(Exclusive Table Lock 발생 등의 부작용 수반)나 `MERGE INTO` 구문을 Type-Safe하게 처리할 수 있다는 점이 장점이다.
...
- **정상적인 트랜잭션 경계**: `@Transactional`이 전체 작업 외부에 선언되어 있거나 루프가 종료된 뒤 1회 Commit하면, 10만 건의 INSERT가 단건으로 쪼개져 전송되었든 배치로 전송되었든 Oracle 엔진 레벨의 `user commits`는 **단 1회**만 발생한다.
- **Loop 내부 Commit 또는 AutoCommit**: Loop 안에서 개별 메서드를 호출하며 트랜잭션이 열리고 닫히거나 Connection이 `autoCommit(true)` 상태라면 매 INSERT마다 Commit이 발생한다.
  - Oracle은 Commit 요청을 받는 즉시 LGWR(Log Writer)가 Redo Log Buffer의 변경 벡터를 디스크의 온라인 Redo Log 파일에 기록할 때까지 해당 세션을 대기시킨다.
  - 이 대기 이벤트가 바로 `log file sync`다. 
  - 디스크 I/O Latency가 1ms라고 가정할 때, 10만 번 Commit하면 순수 대기 시간만으로 100,000회 × 1ms = 약 100초가 소요된다.

---

### 2.6. Java 코드에서 모아서 커밋해도 건건이 Commit이 유발되는 원인

개발자가 Java 코드에서는 분명히 "루프가 끝난 뒤 모아서 1회 Commit"을 의도했음에도 불구하고, 실제 Oracle 엔진 레벨에서는 건별로 Commit이 유발되는 경우가 있다. 이는 HikariCP, ojdbc 드라이버, Spring 트랜잭션 프록시의 물리적 동작 특성에서 비롯된다.

#### 1) HikariCP의 Connection 반납 시 `setAutoCommit(true)` 강제 Commit (JDBC 표준 스펙)
...
- **내부 동작**: HikariCP의 기본 풀 설정은 `isAutoCommit: true`다. Spring의 `@Transactional`이나 수동 트랜잭션(`con.setAutoCommit(false)`)으로 작업하던 도중 예외가 발생하거나, 개발자가 명시적인 `commit()`/`rollback()`을 누락한 채 Connection을 반납(`con.close()`)할 때 문제가 발생한다.
- **결과**: HikariCP는 Connection을 풀의 기본 상태로 되돌리기 위해 내부적으로 `connection.setAutoCommit(true)`를 호출한다. 이때 JDBC 드라이버는 JDBC 표준 스펙에 따라 **아직 커밋되지 않은 트랜잭션을 그 즉시 강제 COMMIT**하여 Oracle 엔진으로 전송한다. 이로 인해 롤백되어야 할 데이터가 커밋되거나 의도치 않은 Commit 횟수 증가를 유발한다. (HikariCP 3.x 이상에서는 풀 반납 시 Rollback을 선행하도록 방어 로직이 추가되었으나, 구버전이거나 커스텀 풀 제어 시 빈번히 발생할 수 있다.)

#### 2) Oracle JDBC 드라이버(ojdbc)의 AutoCommit 상태 Desynchronization 버그
Oracle JDBC 드라이버의 특정 구버전(ojdbc6, ojdbc7, ojdbc8 12.1~12.2 초기 빌드 등)에서는 드라이버 내부 상태와 Oracle 서버 C 커널 세션 간에 AutoCommit 플래그가 어긋나는 버그(MOS Doc ID 2038739.1 등)가 존재했다.
- **발생 메커니즘**: Java 코드에서 `setAutoCommit(false)`를 정상 호출했음에도, 드라이버 레벨의 Statement 캐싱(`oracle.jdbc.implicitStatementCacheSize`)을 사용하거나 ORA 에러 발생 후 복구되는 과정에서 드라이버의 트랜잭션 상태 추적 플래그 변경이 발생한다.
- **결과**: 드라이버는 autoCommit이 켜져 있는 것으로 잘못 판단하고, 매 `execute()` Call마다 Oracle Net8 프로토콜 패킷에 'Auto-Commit bit'(`kpoal8` 커밋 플래그)를 실어 보낸다. Java 코드 상에서는 단 1회의 Commit도 호출하지 않았는데 Oracle 엔진은 매 실행마다 1건씩 Commit을 수행하여 `log file sync`가 급증한다. (최신 ojdbc8/ojdbc11 19.3 이상 패치셋에서는 해결된 상태다.)

#### 3) Spring 트랜잭션 프록시 누락과 AOP Self-Invocation
...
- **AOP Self-Invocation**: 동일한 `@Service` 클래스 내부에서 `this.processChunk()` 형태로 `@Transactional` 메서드를 호출하면 Spring AOP 프록시를 거치지 않고 대상 객체가 직접 호출된다. 그 결과 `@Transactional`이 완전히 무시된 채 JDBC 기본 모드(Auto-Commit: true)로 실행되어 매 쿼리마다 즉시 Commit된다.

---

## 3. 10046 Extended SQL Trace로 단건(r=1) vs 배치(r=1000) 확인하는 방법

실제로 데이터가 어떻게 처리되고 있는지 Oracle DB 레벨에서 확인하는 방법으로 **10046 Extended SQL Trace**가 있다.

### 3.1. 10046 Trace의 개념 및 진단 레벨
10046 이벤트는 Oracle이 내부적으로 제공하는 확장 SQL Trace 이벤트다. 표준 SQL Trace(`SQL_TRACE=TRUE`)가 SQL Parsing, Fetch Executions와 CPU/I/O 총량만 기록하는 반면, 10046 이벤트는 **바인드 변수 실측값**과 **대기 이벤트(Wait Event)**를 마이크로초 단위로 기록한다.

| 진단 레벨 | 수집 내용 | 용도 |
| :--- | :--- | :--- |
| **Level 1** | 표준 SQL Trace (Parse/Execute/Fetch 통계, 처리 행 수) | 기본 실행 통계 확인 |
| **Level 4** | Level 1 + **바인드 변수 값 (Bind Variables)** | 바인드 파라미터 확인 |
| **Level 8** | Level 1 + **대기 이벤트 (Wait Events 및 대기 시간)** | I/O 및 Lock 경합 분석 |
...
실무 환경에서는 개발자 본인의 SQL 클라이언트 세션보다, Spring WAS(HikariCP)나 배치 서버가 맺고 있는 **다른 세션**을 대상으로 10046 트레이스를 걸어야 하는 경우가 대부분이다. Oracle은 DBA 세션에서 다른 활성 세션을 지정해 10046 트레이스를 켜고 끌 수 있는 메커니즘을 제공한다.

##### 1) `DBMS_MONITOR` 패키지 사용 (표준 권장 방식)
대상 애플리케이션 세션의 `SID`와 `SERIAL#`를 `V$SESSION`에서 조회한 뒤 다른 세션에서 활성화한다.
...
##### 2) HikariCP Connection Pool 환경: 모듈 또는 클라이언트 식별자 단위 추적
WAS의 Connection Pool 환경에서는 작업이 어떤 세션(SID)으로 진입할지 사전에 특정하기 어려운 경우가 많다. 이 경우 세션 ID 대신 Spring에서 부여한 모듈명이나 클라이언트 식별자 단위로 트레이스를 걸 수 있다.
...
#### HikariCP + Spring Batch 멀티 스텝 환경: SID가 계속 변경될 때의 대응 방안
실제 Spring Batch 운영 환경에서는 Chunk 단위마다 트랜잭션이 열리고 닫히며 Connection이 HikariCP 풀에 Check-in된 후 다시 Check-out된다.  
단일 스레드 환경이라면 HikariCP의 LIFO(후입선출) 특성상 방금 Check-in된 Connection(동일 SID)이 즉시 다시 할당될 수도 있지만, 다음과 같은 조건에서는 **물리 Connection인 Oracle SID가 실행 도중 언제든 변경**될 수 있다:

1. **멀티 스레드 / 파티셔닝**: 여러 스레드가 동시에 서로 다른 Connection을 꺼내 병렬 처리하므로 SID가 세션 풀 전체로 분산된다.
2. **공용 Connection Pool 경합**: 웹 요청이나 다른 비즈니스 로직과 Connection Pool을 공유하는 경우, Chunk 반납 직후 다른 스레드가 해당 Connection을 가져가면 다음 Chunk는 다른 SID를 할당받는다.
3. **`maxLifetime` 만료**: 수천만 건을 처리하는 장시간 배치 도중 HikariCP Connection 최대 수명이 만료되면 물리 Connection이 재생성되며 SID가 교체된다.

따라서 특정 SID 번호 하나만 믿고 `SESSION_TRACE_ENABLE(session_id => 142)`를 걸어두면, 작업이 다른 세션으로 넘어가는 순간 트레이스 로그가 누락된다. 이에 대해서는 다음 방식으로 대응해야 한다.

##### 1) `CLIENT_IDENTIFIER` 기반 트레이스와 `trcsess` 병합 (표준 권장)
Connection Pool이 어떤 물리 세션(SID)을 할당하든 상관없이, 애플리케이션 컨텍스트에서 Oracle 식별자를 주입하고 해당 식별자 전체에 트레이스를 건다. 식별자를 주입하는 방법은 **`application.yml` 설정**과 **Java 코드(`StepExecutionListener`)** 둘 다 지원하며, 운영 목적에 맞춰 선택할 수 있다.

###### 방법 A: `application.yml` 설정
Java 코드를 수정하거나 재빌드할 필요 없이, HikariCP가 물리 Connection을 맺을 때 Oracle 세션에 식별자를 즉시 등록하도록 설정한다.

```yaml
spring:
  datasource:
    hikari:
      # 물리 Connection 생성 즉시 Client ID 주입
      connection-init-sql: "BEGIN DBMS_SESSION.SET_IDENTIFIER('BATCH_ORDER_STEP'); END;"
```
- **동작 원리**: HikariCP가 DB와 소켓을 열고 물리 Connection을 풀에 넣을 때 위 PL/SQL을 1회 실행한다. Oracle C 커널 내부의 세션 메모리(UGA)에 `client_identifier`가 즉시 기록되며 `V$SESSION.CLIENT_IDENTIFIER`에 영구 반영된다.
- **장점**: 코드 배포 없이 설정 파일만으로 즉시 적용 가능.
- **주의점**: Connection Pool 전체에 적용되므로, 웹 API와 배치가 Connection Pool을 공유하는 공용 DataSource 환경보다는 **배치 전용 DataSource/프로파일**에 적용하는 것이 안전하다.

###### 방법 B: Java `StepExecutionListener` 설정 (스텝별 정교한 분리)
...
public class OracleTraceStepListener implements StepExecutionListener {
    ...
    public void beforeStep(StepExecution stepExecution) {
        // Step 시작 시 현재 물리 Connection에 Client ID 주입
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        // Step 종료 후 Connection Pool에 Check-in될 때 식별자 초기화 (풀 오염 방지)
        try (Connection con = DataSourceUtils.getConnection(dataSource)) {
...
```
- **동작 원리**: JDBC 4.0 표준 `Connection.setClientInfo()`를 호출하면, Oracle JDBC 드라이버(Thin Driver)는 별도의 왕복 쿼리를 던지지 않고 다음 번 SQL Call을 보낼 때 Net8 프로토콜 패킷 헤더에 해당 메타데이터를 **Piggyback**하여 Oracle 엔진에 전송한다.
- **장점**: Step 1(`READ_STEP`), Step 2(`INSERT_STEP`)처럼 스텝 단위로 식별자를 동적으로 분리할 수 있다.

---

###### 두 방식의 동작 비교 요약
| 구분 | `application.yml` (`connection-init-sql`) | Java `StepExecutionListener` |
| :--- | :--- | :--- |
| **완벽 동작 여부** | **완벽 동작** (Oracle 10g~23ai 표준) | **완벽 동작** (Oracle JDBC 드라이버 네이티브 지원) |
| **적용 단위** | Connection Pool(DataSource) 전체 세션 | 개별 Step 단위 (Step별 동적 변경 가능) |
| **코드 수정** | 없음 (yml 설정 및 재시작만 필요) | Listener 클래스 구현 및 등록 필요 |
| **권장 환경** | 배치 전용 애플리케이션의 쿼리/Batching 여부를 일괄 확인할 때 | 멀티 스텝 중 **특정 병목 스텝만 핀포인트로 추적**할 때 |
...
SID가 바뀌어 DB 서버의 서로 다른 `.trc` 파일들에 로그가 쪼개져 저장되더라도, Oracle 공식 유틸리티인 `trcsess`로 Client ID 기준 단일 파일로 병합할 수 있다.
```bash
...
HikariCP가 Connection을 100번 갈아타며 실행했더라도, Oracle 라이브러리 캐시는 세션과 무관하게 동일 `SQL_ID`의 `EXECUTIONS`와 `ROWS_PROCESSED`를 누적 집계하므로 `rows_per_exec` 지표를 통해 Batching 여부를 즉시 검증할 수 있다.
...
| **네트워크 대기** | `SQL*Net ... from client` 10,000회 | `SQL*Net ... from client` 10회 | 네트워크 Roundtrip 지연 시간이 누적되지 않음 |
| **버퍼 블록 Pin** | 1건마다 Pin 획득 $\rightarrow$ 해제 반복 | 블록 Pin 유지 상태에서 연속 삽입 | `cache buffers chains` 래치 경합 급감 |
...
특히 중요한 점은 **데이터 블록 Pin 유지 메커니즘**이다.  
단건(`r=1`)으로 처리하면 Oracle 엔진은 매 레코드마다 테이블 세그먼트의 타깃 블록을 버퍼 캐시에서 찾아 Pin을 꽂고 행을 쓴 뒤 즉시 Unpin한다. 다음 행이 물리적으로 동일한 블록에 저장되더라도 매번 해시 버킷 래치를 획득하고 해제하는 과정을 반복해야 한다.  
반면 Array Processing(`r=1000`)에서는 타깃 블록에 Pin을 꽂은 채 블록의 여유 공간(Free Space)이 허용하는 한 여러 레코드를 연속해서 밀어 넣는다. 그 결과 Current Mode I/O를 나타내는 `cu` 지표가 20분의 1 수준으로 감소한다.

---

### 3.5. Trace 로그로 확인하는 Commit 시점 (`XCTEND`)

10046 Trace 파일에서 트랜잭션의 종료(Commit 또는 Rollback)는 `XCTEND` 마커로 기록된다.
...
XCTEND r=0, rd=0, shake=0, flag=2
```
1만 건이 모두 적재될 때까지 메모리(버퍼 캐시 및 Undo 블록) 상에서만 트랜잭션이 유지되며, 작업이 끝난 뒤 단 1번의 `log file sync`와 `XCTEND`가 찍힌다.

---

## 4. 테이블 단위로 실시간 및 과거 Commit 실측치를 검증하는 방법
...
- **실측 해석**:
  - `EXECUTIONS = 10,000`, `ROWS_PROCESSED = 10,000` $\rightarrow$ `rows_per_exec = 1.00`: 애플리케이션 코드가 어떻게 작성되었든 Oracle 엔진은 **1만 번 단건 실행**한 상태다.
...
Commit이 1건마다 발생했는지, 트랜잭션 종료 시 1번만 발생했는지는 세션 통계에서 확인한다.
...
- **실측 해석**:
  - 1만 건 처리 후 `user commits = 1` $\rightarrow$ 단일 트랜잭션 정상 Commit.
  - `user commits = 10,000` $\rightarrow$ 행마다 개별 Commit 발생.
  - `SQL*Net roundtrips to/from client`가 10,000 이상이면 네트워크 Roundtrip이 병목을 유발하고 있음을 의미한다.
...
- **실측 해석**:
  ...
  - 반면 조회를 할 때마다 트랜잭션 슬롯 번호가 바뀌거나 `used_urec`가 1로만 리셋된다면 건건이 Commit하며 트랜잭션을 닫고 새로 여는 상태다.
...
#### 2) `DBA_HIST_ACTIVE_SESS_HISTORY` (AWR ASH): 과거 Commit 대기 이벤트 집중도 분석
...
```
단건 Commit이 발생했다면 `log file sync` 이벤트가 압도적인 샘플 수로 집계된다.
...
#### 4) `FLASHBACK_TRANSACTION_QUERY`: 테이블 단위 과거 트랜잭션별 실제 Commit 이력 추적
Oracle의 Undo 보존 기간(`undo_retention`) 내라면, 플래시백 트랜잭션 쿼리를 통해 특정 테이블에 발생한 트랜잭션 XID와 실제 Commit 시각, 처리 건수를 1건 단위로 복원하여 검증할 수 있다.
...
- **실측 해석**:
  - **단일 Commit 증명**: 1개의 `xid`에 대해 `affected_rows = 10,000`이고 1개의 `commit_timestamp`만 출력된다. 1만 건이 단일 트랜잭션으로 Commit되었음을 물리적으로 확증할 수 있다.
  - **건건이 Commit 증명**: 서로 다른 수천~수만 개의 `xid`가 출력되고, 각각 `affected_rows = 1`이며 마이크로초 단위로 분리된 `commit_timestamp`가 찍힌다. 애플리케이션의 트랜잭션 경계 설정 오류를 입증할 수 있다.
...
| **JPA 프로퍼티** | Batching 옵션 활성화 | `batch_size: 500~1000`<br/>`order_inserts: true` | 미설정 시 JDBC addBatch 미호출 |
| **MyBatis 매퍼** | 배치 실행 모드 | `ExecutorType.BATCH` 모드 사용 | XML `<foreach>` 기반 다중 INSERT 지양 (파싱/바인드 한도 초과 위험) |
| **트랜잭션 경계** | Commit 발생 주기 | 루프 외부 1회 Commit (`user commits = 1`) | 루프 내 Commit 발생 시 `log file sync` 경합 유발 $\rightarrow$ 트랜잭션 범위 재조정 |
| **Oracle 실측 지표** | 실행당 행 수 | `rows_per_exec` $\approx$ `batch_size` | `rows_per_exec = 1`이면 프레임워크 Batching 무력화 상태 확인 |
...
애플리케이션 계층에서 리스트나 컬렉션을 넘기는 코드는 개발 편의성을 위한 추상화일 뿐, 데이터베이스 엔진 수준의 물리적 Batching을 보장하지 않는다.  
특히 JPA의 `IDENTITY` 전략과 Hibernate 쓰기 지연 메커니즘의 충돌, MyBatis XML `<foreach>`의 파싱 부하 등은 겉으로 드러나지 않는 대표적인 성능 저하 원인이다.
