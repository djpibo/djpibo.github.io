...
올영세일과 같은 대규모 프로모션 기간에는 오프라인 매장과 온라인몰 전반에 걸쳐 평시대비 수배에서 수십 배에 달하는 트랜잭션이 한꺼번에 집중됩니다.

이 시점에 데이터베이스 서버의 CPU 점유율이 높게 치솟는 상황을 마주했을 때, 단순히 기술적으로 어떤 SQL이 느린지를 찾아내는 것에서 끝나서는 안 됩니다.  
**"지금 이 순간 시스템에서 수행되는 비즈니스 업무가 얼마나 중요한 업무이며, 그 업무가 원활하게 완수될 수 있도록 기술적 비효율을 어떻게 포착하고 개선해 낼 것인가?"**가 핵심입니다.

이번 회고 1편에서는 시간대별 AWR 및 ASH 통계(Stat)를 기반으로 비효율을 포착하고, 개발 스쿼드와의 면밀한 도메인 로직 분석을 통해 개선을 이끌어낸 **두 가지 핵심 업무 최적화 사례**를 정리합니다.

---

## 1. 비즈니스 맥락: 7천억 원의 매출 흐름과 매장 현장의 핵심 업무

올영세일 기간 동안 시스템이 뒷받침해야 하는 핵심 비즈니스는 크게 두 축으로 나뉩니다.

```mermaid
graph TD
    subgraph Flow["옴니채널 7,000억 원 매출 흐름 및 물류 파이프라인"]
        A["온라인몰 결제 / 매출 발생"] --> B["온라인몰 재고 차감"]
        B --> C["중앙 물류창고 출하지시<br/>(각 매장 및 MFC 센터 재고 이동)"]
        D["전국 1,300+ 매장 POS 결제"] --> E["실시간 매장 POS 매출 집계"]
    end

    subgraph Store["매장 현장의 4대 핵심 진열 및 ESL 작업"]
        F1["전략상품 진열"]
        F2["보충 진열"]
        F3["타워 매대 진열"]
        F4["클리어런스 진열"]
        F1 & F2 & F3 & F4 --> G["매대 진열 변경 발생"]
        G --> H["전자라벨(ESL) 어사인 / 언어사인 표시 교체"]
    end
```

### 1.1. 총 7,000억 원 규모의 거래 및 물류 흐름
세일 기간 동안 온라인몰 출하지시와 전국 매장 POS 매출 집계가 맞물려 **총 7,000억 원 규모의 거래와 현금 흐름**이 발생합니다.
- **온라인몰**: 고객 주문 결제 시 즉시 온라인몰 재고를 차감한 후, 각 매장 혹은 MFC(도심형 마이크로 풀필먼트 센터)로 상품을 적시 보충하기 위해 중앙 물류창고에서 출하지시가 쉴 새 없이 일어납니다.
- **오프라인 매장**: 전국 1,300여 개 매장에서 발생하는 실시간 POS 결제 데이터가 매출로 확정되고 집계됩니다.

### 1.2. 오프라인 매장 현장의 4대 진열 업무와 전자라벨(ESL)
...
4. **클리어런스(Clearance) 진열**: 한정 수량 및 특가 상품의 매대 전환

매대 진열 변경이 일어날 때마다 매장 단말기를 통해 상품과 전자가격표 태그를 연결하는 **어사인(Assign)**, 혹은 기존 연결을 해제하는 **언어사인(Unassign)** 표시 교체 작업이 수반됩니다.  
...
대규모 트래픽 분석 시 가장 주의해야 할 부분은 **세일 기간 전체 누적 통계(예: 7일간의 AWR Top SQL 요약)에만 의존하는 것**입니다.

```mermaid
timeline
    title 올영세일 주요 업무 트래픽 흐름
    오전 : 매장 오픈 준비 & 1차 진열 (ESL 라벨 어사인/언어사인 집중)
    낮/오후 : 고객 유입 증가 & 보충/타워/클리어런스 매대 변경 및 온라인 주문 급증
    저녁/피크 : 퇴근 시간대 매장 POS 매출 결제 및 실시간 집계 집중
    야간/마감 : 온라인몰 재고 차감에 따른 물류창고 출하지시(매장/MFC) & 일마감 정산
```
...
- **도메인 처리 대비 비효율의 은폐**: 실제 비즈니스 트랜잭션 건수(매출 N건, 어사인 M건)에 비해 쿼리 호출이 몇 배로 중복 발생하고 있는지 여부는 누적 통계만으로는 분리해 내기 어렵습니다.

따라서 1시간(AWR 스냅샷) 혹은 수분 단위로 구간을 쪼개어 변화량(Delta)을 산출하고, **해당 시간대의 실제 비즈니스 처리 건수와 쿼리 실행 횟수(`executions_delta`)를 대조하는 시간대별 Delta 분석**이 필수적입니다.
...
### 3.2. ASH(`DBA_HIST_ACTIVE_SESS_HISTORY`)를 이용한 단기 집중 부하 추적
...
시간대별 Delta 분석을 통해 첫 번째로 포착한 비효율은 **매장 POS 매출 집계 시간대에 실제 매출 인식 처리량 대비 과도하게 발생하는 쿼리 군집**이었습니다.
...
flowchart LR
    ...
    subgraph After["[개선 후] 도메인 분석 기반 쿼리 최적화"]
        ...
        A2 --> A3["불필요 로직 제거 및 쿼리 최적화<br/>(개발 스쿼드 협업)"]
        A3 --> A4["매출 인식 경로 경량화 & 리소스 안정화"]
    end
```

### 4.1. 포착된 비효율 지표 (Stat)
- 오프라인 매장 매출 결제가 집중되는 피크 시간대에, 실제 확정된 **POS 매출 전표 건수에 비해 특정 조회 및 검증 SQL의 실행 횟수가 비정상적으로 높게 형성**되어 상위 CPU를 점유하고 있었습니다.
- 1회당 CPU 소모 시간은 수 밀리초에 불과했으나, 대규모 세일 매출 트랜잭션과 맞물리며 초당 수천~수만 회 단위로 누적되어 CPU 리소스를 잠식하고 있었습니다.
...
## 5. [개선 사례 2] 매대 진열 변경 시 전자라벨(ESL) 표시 로직 중복 호출 개선

두 번째로 포착한 과제는 **매대 진열 변경이 일어날 때마다 실행되는 전자라벨(ESL) 표시 교체 로직의 중복 호출**이었습니다.

```mermaid
flowchart LR
    subgraph Before["[개선 전] 라벨 표시 교체 시 중복 호출"]
        direction TB
        E1["매대 진열 변경<br/>(전략/보충/타워/클리어런스)"]
...
### 5.1. 포착된 비효율 지표 (Stat)
- 매장 오픈 준비 시간대(08~09시)와 오후 매대 재배치 시간대에 매대 관리 및 전자라벨 매핑 관련 SQL이 시간대별 Top CPU 리스트에 집중적으로 등장했습니다.
...
- 이때 단말기로 상품과 전자라벨을 매핑하는 **어사인(Assign)** 및 기존 매핑을 푸는 **언어사인(Unassign)** 작업이 실행되는데, 로직 호출 흐름에 비효율이 존재했습니다:
  - 매대 내 일부 상품만 변경되었음에도 해당 영역과 연관된 라벨 표시 로직이 광범위하게 재호출되는 현상.
  - 연속된 진열 변경 과정에서 동일한 ESL 태그에 대해 상태 변경 쿼리가 중복으로 발생하는 패턴.
...
- **개선 결과**:
  - 전자라벨 연동 관련 쿼리의 실행 횟수(Executions)를 70% 이상 감축.
...
quadrantChart
    ...
    quadrant-1 "고비용 다빈도 (즉시 개선 대상)"
    quadrant-2 "Heavy SQL (대용량 집계 / 정산)"
    quadrant-3 "경미한 쿼리"
    quadrant-4 "초다빈도형 (호출 중복 / 로직 비효율)"
    "POS 매출 인식 수반 쿼리": [0.90, 0.20]
    "ESL 라벨 어사인/언어사인 쿼리": [0.85, 0.25]
    "물류창고 출하지시 배치": [0.25, 0.85]
    "일마감 매출 원장 정산": [0.15, 0.90]
```

### 6.1. 유형 A: 다빈도 호출형 (High Executions, Low CPU per Exec)
- **대표 사례**: POS 매출 인식 시 수반되던 중복 검증 쿼리, 매대 변경 시 ESL 라벨 중복 호출 쿼리
- **특징**: 1회 실행 시간은 수 밀리초로 극히 짧아 평시에는 전혀 눈에 띄지 않지만, 대규모 프로모션 상황에서 초당 수천 회 이상 폭증하며 데이터베이스 전체 CPU의 상당 부분을 차지함.
- **해결 방안**: 단일 SQL 튜닝(인덱스 변경 등)으로는 근본적인 한계가 존재함. **개발 스쿼드와 도메인 로직을 분석하여 불필요한 호출 자체를 제거하거나 일괄 집합 처리로 전환**해야 함.

### 6.2. 유형 B: 고비용 연산형 (Low Executions, High CPU per Exec)
- **대표 사례**: 중앙 물류창고 출하지시(매장/MFC) 배정 쿼리, 야간 매출 원장 일마감 집계 쿼리
- **특징**: 실행 횟수는 시간당 수십~수백 건 수준이나, 1회 수행 시 대량의 블록을 읽고 복잡한 연산을 수행하여 CPU 점유 시간이 김.
- **해결 방안**: 결합 인덱스 구성, 파티션 프루닝, 인라인 뷰 조건 침투(Pushdown) 등 **옵티마이저 실행 계획 최적화 중심의 데이터베이스 튜닝**으로 해결.
...
2. **개발 스쿼드와의 원팀 협업**: 비효율을 포착하는 것은 데이터베이스 지표(Stat)였지만, 문제를 해결한 열쇠는 도메인 로직을 가장 잘 알고 있는 개발 스쿼드와 함께 머리를 맞대고 중복 로직을 덜어낸 협업 과정에 있었습니다.
3. **업무 수행의 가치 실현**: 
...
다음 편([2편: 비효율 패턴 파악하기 (Insight)](./oy-sale-insight-inefficient-patterns))에서는 이번에 포착한 SQL들이 내부적으로 품고 있었던 **구조적 비효율 안티패턴(단건 루프 I/O, 인라인 뷰 조건 침투 실패 등)**을 상세한 실행 계획과 함께 심층 해부합니다.
