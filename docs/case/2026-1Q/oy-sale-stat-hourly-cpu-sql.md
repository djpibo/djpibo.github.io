...
이번 회고 1편에서는 시간대별 AWR 및 ASH 통계를 기반으로 비효율을 포착하고, 개발 스쿼드와의 면밀한 도메인 로직 분석을 통해 개선을 이끌어낸 **두 가지 핵심 업무 최적화 사례**를 정리합니다.
...
graph TD
    subgraph Flow["옴니채널 7,000억 원 매출 흐름 및 물류 파이프라인"]
        ...
        B --> C["중앙 물류창고 출하지시<br/>매장 및 MFC 재고 이동"]
        D["전국 1,300+ 매장 POS 결제"] --> E["실시간 매장 POS 매출 집계"]
    end

    subgraph Store["매장 현장의 4대 핵심 진열 및 전자라벨 작업"]
        F1["전략상품 진열"]
        F2["보충 진열"]
        F3["타워 매대 진열"]
        F4["클리어런스 진열"]
...
- **도메인 처리 대비 비효율의 은폐**: 실제 비즈니스 트랜잭션 건수 대비 쿼리 호출이 몇 배로 중복 발생하고 있는지 여부는 누적 통계만으로는 분리해 내기 어렵습니다.

따라서 1시간 스냅샷 혹은 수분 단위로 구간을 쪼개어 변화량을 산출하고, **해당 시간대의 실제 비즈니스 처리 건수와 쿼리 실행 횟수(`executions_delta`)를 대조하는 시간대별 분석**이 필수적입니다.
...
- 이때 단말기로 상품과 전자라벨을 매핑하는 **어사인** 및 기존 매핑을 푸는 **언어사인** 작업이 실행되는데, 로직 호출 흐름에 비효율이 존재했습니다:
  ...
  - 연속된 진열 변경 과정에서 동일한 전자라벨 태그에 대해 상태 변경 쿼리가 중복으로 발생하는 패턴.
